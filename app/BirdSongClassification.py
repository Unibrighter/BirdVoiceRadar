...
sys.path.insert(0, '..')  # add the parent directory into the system path, in order to import config
import config
...
class BirdSong:
    ...
    def predict_one_bird(self, mfcc_feature, gmm):
        ...
        for label, agmm in gmm.items():
            ...
            score_dictionary[get_bird_code(label)] = likelihood
            sorted_score_dictionary = sorted(score_dictionary.items(), key=operator.itemgetter(1), reverse=True)

            if likelihood > best_likelihood:
                best_likelihood = likelihood
                best_label = label

        # this should be altered into a dictionary
        ...
        return sorted_score_dictionary, confidence
